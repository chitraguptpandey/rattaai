...
const TableData = props => {
    ...
    const renderActionIcons = () => {
        ...
        props.actions.map((action, i) => {
            lstActions.push(
                <TouchableWithoutFeedback onPress={() => action(props.data[props.pkid])}>
                    <MaterialIcons key={i} name={props.actionIcons[i]} size={20} style={styles.actionIcon}
        ...
        props.fields.map((item, index) => {
            arrCols.push(
                <View key={index} style={{
                    ...
                    {
                        item.indexOf('+', 0) == -1 && item.indexOf('^', 0) == -1 &&
                        <Text style={styles.dataText}>
                            {
    ...
    return (
        ...
        props.triggerOnLongPress && props.options != undefined && props.options.length > 0 ?
            <Menu renderer={SlideInMenu}>
                <MenuTrigger triggerOnLongPress={true} onPress={() => setClickedId(props.data[props.pkid])}>
...

                {
                    ...
                    <View style={{ ...styles.dataRow, ...styles.leftLine, flex: colSizes.length > 0 ? colSizes[colSizes.length - 1] : 0.5 }}>
                        <Menu renderer={ContextMenu}>
                            <MenuTrigger onPress={() => setClickedId(props.data[props.pkid])}>
                                <View style={styles.dotsContainer}>
                                    <Ionicons name="md-more" size={25} color={Colors.black2} />
                                </View>
                            </MenuTrigger>
                            <MenuOptions customStyles={optionsStyles}>
                                {renderMenuOptions()}
