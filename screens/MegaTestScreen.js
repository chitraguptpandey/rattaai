...
const MegaTestScreen = props => {
	...
	const getData = async () => {
		setIsLoading(true);
		try {		
			const apiData = await dispatch(testActions.getList(Variables.ExamId, Variables.BatchId, false,0));
			setTests(apiData);
	...
	const openTest = (test) => {
		GlobalFunctions.navigate(props, 'TestInstruction', { testId: test.id, testName: test.name, instructions: test.instructions, duration: test.duration });
	}
			...
			arrItems.push(
				<View key={index}>
						...
						<View style={{ ...GS.row100 }}>
							<View style={{ ...GS.f1, ...GS.row }}>
								...
								<View style={{...GS.m10}}>
									...
									<Bold numberOfLines={1}  style={{ ...GS.fs16, ...GS.mb5 , textTransform: 'capitalize' }}>
										{item.name}
									</Bold>
							...
							<View style={{ ...GS.f1, ...GS.aright }}>
								...
								<Text style={{ ...GS.fs14, ...GS.textPrimaryDark }}>TOTAL MS: {item.total_marks}</Text>
								<View style={{ ...GS.row100,...GS.jright,...GS.mt15 }}>
									<FontAwesome5 name="info-circle" size={24} color="#1974D2" onPress={() => textinfo(item)} />

									<View style={{ ...GS.bgLightGreen, ...GS.rounded20, ...GS.ph15, ...GS.h20, ...GS.jcenter, ...(item.test_date != "" ? GS.ml10 : undefined) }}>
										<Bold style={{ ...GS.textWhite, ...GS.fs12 }}>{item.id}</Bold>
									</View>
								</View>
					...
					<View style={{ flex: 1.2, alignItems: 'flex-end', marginTop: -30, ...GS.mr20 }}>
						<SubmitButton title="Start Test" onPress={() => openTest(item)} style={{ ...GS.h30, ...GS.minw140,  ...GS.rounded20 ,backgroundColor:Colors.blueDark ,...GS.borderBlueDark }}
	...
	if (isLoading) {
		return (
			<MyContainer navigation={props.navigation} title="Mega Test">
	...
	return (
		<MyContainer navigation={props.navigation} title="Mega Test" stickyHeaderIndices={tests.length > 3 ? [0] : undefined} padder={false}>

			{
				...
				<View style={{ ...GS.w100p, ...GS.h55, ...GS.pt5, ...GS.mb5, ...GS.ph10, ...GS.bgWhite, ...GS.roundedBottom10, elevation: 6 }}>
					<MyTextInput name="name" value={formValues} error={formErrors} submitting={isSubmitting}
						initialValue={formValues['name']} showLabel={false} placeholder="Search by paper name..."
