...
const RemedialTestResult = props => {
	...
	const getData = async () => {
		...
		try {
			...
			const apiData = await dispatch(testActions.getList(Variables.ExamId, Variables.BatchId, true, 1));
			setTests(apiData);
	...
	if (isLoading) {
		return (
			<MyContainer navigation={props.navigation} title="Remedial Test Result">
	...
	const instaRClickHandler = (test) => {
		...
		// GlobalFunctions.showInstaRConfirmation(props, test.id, test.test_name, test.total_marks, 2, true, 1);
	...
	const yesSelectInstaRConfirmationBox = (props, testId, testName, langId,teststopbacknavigate) => {
		...
		props.navigation.navigate( 'StartTest' , { testId: testId, langId: langId, testName: testName, instaR: 1,teststopbacknavigate:1 } );
	...
	const noSelectInstaRConfirmationBox = (props, testId, testName, totalMarks, langId, redirect,teststopbacknavigate) => {
		...
		if (redirect) {
			...
			props.navigation.navigate( 'ResultView', { testId: testId, langId: langId, testName: testName, totalMarks: totalMarks ,instaR: 1,teststopbacknavigate:1 ,resultbackscreenNo : 1 });
		}
	}
	const openTest = (test) => {

		GlobalFunctions.navigate(props, 'ResultView', { testId: test.id, langId: 2, testName: test.test_name, totalMarks: test.total_marks, resultbackscreen: 1 });
	}
						...
						<CardView style={styles.testContainer}>
							<View style={{ ...GS.row100, ...GS.f1 }}>
								<View style={{ ...GS.f1 }}>
									...
									<View style={{ ...GS.row100, ...GS.f1 }}>
										<View style={{ ...GS.h65, ...GS.w65, ...GS.acenter }}  >
											<Image source={require('../assets/Frame.jpg')} />
										</View>
										<View style={{ ...GS.ph5, ...GS.pl10 }}>
											<Text style={{ ...GS.fs13, ...GS.textDarkGray }}>{item.question_ids.split(',').length} QUESTIONS</Text>

											<Bold numberOfLines={1} style={{ fontSize: 13 }}>
												{item.test_name}
											</Bold>
											<Text style={{ ...GS.pv2, ...GS.fs13, color: Colors.darkGray }}>Score: {item.score}</Text>
											{
												item.instar_status == "1" &&
												<Text style={{ ...GS.fs13, color: Colors.darkGray }}>IR Score: {item.instar_score}</Text>
											}
										</View>
								...
								<View style={{ ...GS.jcenter, ...GS.f1 }}>
									...
									<View style={{ ...GS.jcenter, ...GS.row100, ...GS.f1 }}>
										<View style={{ ...GS.f1, ...GS.acenter, ...GS.jcenter }}>
											{
	...
	return (
		<MyContainer navigation={props.navigation} title="Remedial Test Result" stickyHeaderIndices={tests.length > 3 ? [0] : undefined} padder={false}>
			...
			<MyDialog isVisible={showInstaConfirmation} hideDialog={() => setShowInstaRConfirmationBox(false)} titleStyle={{ ...GS.textWhite }}
				...
				<View style={{ ...GS.h60 }}>
					<SubmitButton title="हां" onPress={() => yesSelectInstaRConfirmationBox(props, irTest.id, irTest.test_name, irTest.total_marks, 2, true, 1)} textStyle={{ ...GS.textWhite }} style={{ ...GS.bgheadingColor, ...GS.minw75, ...GS.f1, ...GS.mb10, ...GS.border3, ...GS.borderWhite }} />
				</View>
				<View style={{ ...GS.h60 }}>
					<SubmitButton title="नहीं" onPress={() => noSelectInstaRConfirmationBox(props, irTest.id, irTest.test_name, irTest.total_marks, 2, true, 1)} textStyle={{ ...GS.textWhite }} style={{ ...GS.bgheadingColor, ...GS.minw75, ...GS.f1, ...GS.mb10, ...GS.border3, ...GS.borderWhite }} />
				</View>
			</MyDialog>
			{
				tests.length > 0 ?
...
export default RemedialTestResult;
