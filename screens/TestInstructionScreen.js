...
const TestInstructionScreen = props => {
    ...
    const startTestClickHandler = () => {
        GlobalFunctions.navigate(props, 'StartTest', { testId: testId, langId: formValues['language_id'], testName: testName, duration: duration, instaR : 0, screen: 0 });
    }
    ...
    return (
        <MyContainer navigation={props.navigation} title={testName}>
            ...
            <Text style={styles.testHeading}>Mega Test</Text>
            <CardView style={GS.ph20}>
                <Bold style={{...GS.fs16 , ...GS.textDarkGray}}>Test Instruction</Bold>
                <Text style={{ ...GS.pv10, ...GS.fs15 }}>
                    {instructions}
                </Text>

                <SubmitButton title="Start Test" IsLoading={showButtonLoader} onPress={startTestClickHandler}  textStyle={{ ...GS.textWhite }} style={{ ...GS.bgheadingColor, ...GS.rounded50, ...GS.minw75, ...GS.mh10, ...GS.f1, }} />
                <CancelButton title="Cancel" onPress={moveToTestList} textStyle={{ ...GS.textWhite }} style={{ ...GS.bgBlueDark, ...GS.mh10,...GS.rounded50, ...GS.f1, ...GS.mb10, ...GS.mt10 }}  />
